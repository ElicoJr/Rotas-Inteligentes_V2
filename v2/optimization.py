...
class MetaHeuristica:
    def __init__(self, equipe_row, pend_tec, pend_com, limite_por_equipe=15):
    ...
    def _vroom(self, df_jobs):
        ...
        vehicle = {
            ...
            "start": [lon_e, lat_e],
            "end":   [lon_e, lat_e],
            # arrivals do VROOM são relativos a t=0 -> transformamos com base em t0
            "time_window": [0, int((pd.to_datetime(self.equipe["fim_turno"]) - t0).total_seconds())]
        }
        ...
        if steps:
            # construir mapas job_id -> ETA relativa, e end_arrival
            eta_map = {}
            end_arrival_s = None
            for st in steps:
                st_type = st.get("type")
                arr_s   = st.get("arrival")
                if arr_s is None or not isfinite(arr_s):
                    continue
                if st_type == "job":
                    jid = int(st.get("job"))
                    eta_map[jid] = t0 + timedelta(seconds=int(arr_s))
                elif st_type == "end":
                    end_arrival_s = int(arr_s)

            # aplica ETA direto (sem merge) pra evitar _x/_y
            ...
            # chegada à base a partir de 'end'
            if end_arrival_s is not None:
                df_jobs_tagged["fim_turno_estimado"] = t0 + timedelta(seconds=end_arrival_s)

            return resp, df_jobs_tagged

        # ---- Fallback 1: OSRM (ordem do DF) ----
        try:
            return resp, _osrm_eta_etd(
                self.osrm, df_jobs_tagged, self.equipe["inicio_turno"], lon_e, lat_e
            )
        except Exception:
            pass

        # ---- Fallback 2: Haversine (ordem do DF) ----
        return resp, _haversine_eta_etd(
            df_jobs_tagged, self.equipe["inicio_turno"], lon_e, lat_e
        )
    ...
    def otimizar_para_equipe(self):
        ...
        for meta_col in ["dthaps_ini","dthaps_fim_ajustado","inicio_turno","fim_turno"]:
...
# ===== Helpers ETA/ETD =====
def _osrm_eta_etd(osrm_client, df_jobs_tagged, inicio_turno_pvh, lon_e, lat_e):
    import pandas as pd
    ...
    coords = [(lon_e, lat_e)]
    coords += [(float(r["longitude"]), float(r["latitude"])) for _, r in df_jobs_tagged.iterrows()]
    ...
    for i, r in df_jobs_tagged.iterrows():
        ...
        chegada = t_cursor + pd.to_timedelta(travel_s, unit="s")
        df_jobs_tagged.at[i, "dth_chegada_estimada"] = chegada

        te_sec = _service_seconds_from_row(r)
        termino = chegada + pd.to_timedelta(int(te_sec), unit="s")
    ...
    df_jobs_tagged["fim_turno_estimado"] = t_cursor + pd.to_timedelta(back_s, unit="s")
    df_jobs_tagged["eta_source"] = df_jobs_tagged["eta_source"].astype("string")
    return df_jobs_tagged

def _haversine_eta_etd(df_jobs_tagged, inicio_turno_pvh, lon_e, lat_e, vel_kmh=30.0):
...
def _padronizar_layout_final(df):
    ...
    for c in ["datasol","dataven","datater_trab","dthaps_ini","dthaps_fim_ajustado",
